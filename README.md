...
- **Video modality** — ✅ shipped in v0.9.0 via Replicate `grok-imagine-video-1.5` (image-to-video) and v0.10.0 via `veed/fabric-1.0` (talking-avatar / lip-sync), inheriting the same cost / sidecar / cache / budget machinery. More video providers (text-to-video, other avatar/lip-sync models) can slot into the same `video` modality next.
