...
async function ensureBudgetFile(filePath: string): Promise<void> {
...
export async function readBudget(): Promise<Budget> {
  ...
  return { ...DEFAULT_BUDGET, ...(JSON.parse(raw) as Partial<Budget>) };
}

export async function writeBudget(updates: Partial<Budget>): Promise<Budget> {
  const filePath = getBudgetPath();
  return withLock(filePath, async () => {
    const current = await readBudget();
    const merged: Budget = { ...current, ...updates };
...
export async function checkBudget(addCost: number): Promise<BudgetCheckResult> {
  if (addCost <= 0) return { block: null, warning: null };

  const budget = await readBudget();
  const session = await readSession();
  const summary = summarize(session);

  const periods: Array<{ period: BudgetPeriod; cap: number | null; current: number }> = [
...
export function formatBudgetBlockError(block: BudgetBlock): string {
  return (
    `Budget cap exceeded: ${block.period} cap is ` +
...
export function formatBudgetWarning(w: BudgetWarning): string {
  return (
    `Budget warning: ${(w.pctUsed * 100).toFixed(0)}% of ${w.period} cap used ` +
    `(${w.currency} ${w.projectedSpend.toFixed(4)} / ${w.currency} ${w.cap.toFixed(2)}).`
  );
}

export function renderBudgetText(budget: Budget): string {
  const fmt = (v: number | null) => (v === null ? "(no cap)" : `${budget.currency} ${v.toFixed(2)}`);
  return [
    `Budget caps:`,
