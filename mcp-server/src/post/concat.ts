...
function codecArgsForOutput(outputPath: string): string[] {
...
export async function concatAudioFiles(inputs: string[], outputPath: string): Promise<void> {
  ...
  const listBody =
    absInputs.map((p) => `file '${p.replace(/'/g, "'\\''")}'`).join("\n") + "\n";
...
export async function transcodeAudio(input: string, output: string): Promise<void> {
  if (!(await ffmpegAvailable())) {
    throw new StructuredError(
      "CONFIG_ERROR",
...
function needsTranscode(sourceMime: string, destPath: string): boolean {
...
export async function saveAudioRespectingPath(
  bytes: Buffer,
  destPath: string,
  sourceMime: string,
...
export async function copyAudioRespectingPath(
  sourcePath: string,
  destPath: string,
  sourceMime: string,
): Promise<{ mimeType: string }> {
