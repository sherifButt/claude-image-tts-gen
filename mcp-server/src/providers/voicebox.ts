...
export class VoiceboxProvider implements TtsProvider {
  readonly id: ProviderId = "voicebox";

  private readonly baseUrl: string;

  constructor(opts: VoiceboxOptions) {
  ...
  async generateSpeech(req: TtsGenRequest): Promise<TtsGenResult> {
    ...
    const engine = (params.engine as string | undefined) ?? undefined;
    const modelSize = (params.model_size as string | undefined) ?? undefined;
    ...
    const initial = (await startRes.json()) as GenerationStatus;
    if (!initial.id) {
      throw new StructuredError(
        "PROVIDER_ERROR",
  ...
  private async pollUntilComplete(generationId: string): Promise<GenerationStatus> {
    ...
    while (Date.now() < deadline) {
      ...
      const state = (await res.json()) as GenerationStatus;
    ...
    throw new StructuredError(
      "PROVIDER_TIMEOUT",
      `Voicebox generation ${generationId} did not complete within ${POLL_TIMEOUT_MS / 1000}s`,
      `Long inputs on CPU can exceed this — try shorter text, a smaller model_size, or run Voicebox with GPU acceleration.`,
    );
  }

  private async fetchAudio(generationId: string): Promise<Buffer> {
...
export async function listVoiceboxProfiles(baseUrl: string): Promise<VoiceboxProfile[]> {
  ...
  try {
    ...
    const data = (await r.json()) as VoiceboxProfile[];
