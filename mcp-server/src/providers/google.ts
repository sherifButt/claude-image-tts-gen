...
export class GoogleProvider implements ImageProvider, TtsProvider {
  ...
  private async generateImageViaImagen(req: ImageGenRequest): Promise<ImageGenResult> {
    if (req.referenceImages && req.referenceImages.length > 0) {
      ...
      throw new Error(
        "Imagen 4 does not accept reference images for generation. Use gemini-2.5-flash-image or gemini-3.1-flash-image-preview for image-to-image, or call images.edit separately.",
      );
    }
