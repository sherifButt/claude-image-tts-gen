...
export function isPocketBuiltInVoice(v: string): boolean {
  return (POCKET_TTS_VOICES as readonly string[]).includes(v);
}
 ...
 */
export function resolvePocketVoice(voiceId: string | null | undefined): string | null {
  if (!voiceId) return null;
  if (isPocketBuiltInVoice(voiceId)) return voiceId;
  if (!existsSync(voiceId)) {
    throw new StructuredError(
      "NOT_FOUND",
      `pocket-tts voice "${voiceId}" is neither a built-in voice nor a file that exists`,
      `Point --voice at a clean reference .wav to clone from, or use one of: ${POCKET_TTS_VOICES.slice(0, 6).join(", ")}, … (run check_pocket for the full list).`,
    );
  }
...
export function pocketReferencePath(voice: string): string {
...
function toBlobPart(b: Buffer): Uint8Array<ArrayBuffer> {
...
export function isCloningUnavailableError(body: string): boolean {
  return /voice cloning/i.test(body);
}

export function cloningUnavailableError(): StructuredError {
  return new StructuredError(
    "PROVIDER_ERROR",
    "pocket-tts loaded the model WITHOUT voice cloning, so it cannot speak in the requested voice",
    "Its cloning weights are a gated repo: accept the terms at https://huggingface.co/kyutai/pocket-tts, run `hf auth login`, then restart the server. Refusing rather than generating — without cloning it would answer in one of its stock voices and nothing downstream would notice.",
  );
}
...
export class PocketTtsProvider implements TtsProvider {
  readonly id: ProviderId = "pocket-tts";
  ...
  async generateSpeech(req: TtsGenRequest): Promise<TtsGenResult> {
    ...
    if (referencePath || !isPocketBuiltInVoice(voice)) {
      ...
      if (!existsSync(path)) {
        throw new StructuredError(
          "NOT_FOUND",
          `pocket-tts reference "${path}" is gone`,
          "It is the voice this audio clones from — restore the file, or pass a built-in voice name instead.",
        );
      }
      /*
    ...
    try {
      ...
      throw new StructuredError(
        "PROVIDER_ERROR",
        `pocket-tts is not reachable at ${this.baseUrl}: ${e instanceof Error ? e.message : String(e)}`,
        "Start it with `pocket-tts serve` (listens on :8000 by default), or set POCKET_TTS_BASE_URL. Run check_pocket to confirm what is working.",
      );
    }

    if (!res.ok) {
      const body = await res.text().catch(() => "");
      if (isCloningUnavailableError(body)) throw cloningUnavailableError();
      throw new StructuredError(
        "PROVIDER_ERROR",
        `pocket-tts returned ${res.status}: ${body.slice(0, 300)}`,
        "Check the pocket-tts server log, or run check_pocket for a full status report.",
      );
    }
    ...
    if (data.length < 1024) {
      throw new StructuredError(
        "PROVIDER_ERROR",
        `pocket-tts returned ${data.length} bytes — too short to be audio`,
        "Check the server log; a very short input line can also produce this.",
      );
    }
    return {
      mimeType: "audio/wav",
   ...
   */
  async probeCloning(): Promise<{ ok: boolean; detail: string }> {
    let seed: Buffer;
    try {
      const form = new FormData();
      form.set("text", "ok");
    ...
    try {
      ...
      if (r.ok) return { ok: true, detail: "cloned a reference successfully" };
