...
import { generateAvatar, type GenerateAvatarArgs } from "./tools/generate-avatar.js";
import { gallery, type GalleryArgs } from "./tools/gallery.js";
...
const imageInputSchema = {
  ...
  properties: {
    ...
    size: {
      ...
      pattern: "^\\d+x\\d+$",
      description:
        "Exact gpt-image-2 output size 'WIDTHxHEIGHT' (overrides resolution + aspectRatio). openai only. Constraints: both edges ÷16, max edge 3840, ratio ≤3:1, 0.65–8.3 MP (e.g. 2048x1152, 3840x2160, 1152x2048). Priced at the nearest 1K/2K/4K tier by megapixels.",
    },
    background: {
      type: "string",
      enum: ["auto", "opaque", "transparent"],
      description:
        "Background handling (gpt-image family / openai). Default auto. transparent needs gpt-image-1 (--model gpt-image-1) — gpt-image-2 rejects it.",
    },
    outputDir: {
      type: "string",
      description:
...
const videoInputSchema = {
  ...
  properties: {
    ...
    imagePath: {
      type: "string",
      description:
        "Path to the input frame to animate. REQUIRED on tier high/ultra (grok is image-to-video only). OPTIONAL on draft/low/normal, where omitting it gives text-to-video straight from the prompt. Generate a still with generate_image if you want to control the opening frame.",
    },
    referenceImagePaths: {
      type: "array",
      items: { type: "string" },
    ...
    tier: {
      ...
      description:
        "Quality/cost ladder, cheapest first. Cost = duration x rate. " +
    ...
    duration: {
      ...
      maximum: 20,
      description:
        "Clip length in seconds. Default 5. Max 20 on draft/low/normal, 15 on high/ultra. Cost = duration x per-second rate.",
    },
    aspectRatio: {
      type: "string",
      enum: ["1:1", "16:9", "9:16", "4:3", "3:4", "3:2", "2:3"],
      description:
        "Output aspect ratio. Omit for the model's auto. 21:9 is not supported. Ignored whenever an input frame is supplied — the frame decides — so this only applies to text-to-video.",
    },
...
const avatarInputSchema = {
  ...
  properties: {
    ...
    audioPath: {
      type: "string",
      description:
        "Path to the speech audio the mouth + head are synced to (mp3/wav/m4a/aac). Required. Generate one with generate_speech. The output video's length equals this audio's length.",
    },
    provider: {
      type: "string",
      enum: ["replicate"],
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      name: "gallery",
      description:
        `Build a self-contained HTML gallery of everything generated by this plugin (images, video, audio) by scanning the output dirs (${config.imageOutputDir}, ${config.audioOutputDir}, ${config.videoOutputDir}) and reading each file's sidecar. Cards show a thumbnail, prompt, model/tier/params, cost, and date, with client-side filter/search/sort. Writes ./gallery.html by default. Re-run to refresh.`,
      inputSchema: {
        type: "object",
        properties: {
...
async function handleVideoCall(args: unknown) {
  const parsed = (args ?? {}) as GenerateVideoArgs;
...
async function handleAvatarCall(args: unknown) {
  const result = await generateAvatar((args ?? {}) as GenerateAvatarArgs, config);
  const lines = [
...
async function handleGallery(args: unknown) {
  const result = await gallery((args ?? {}) as GalleryArgs, config);
  return {
    structuredContent: result,
    content: [{ type: "text", text: result.text }],
  };
...
function handleListProviders(args: unknown) {
  const { modality } = (args ?? {}) as { modality?: "image" | "tts" | "video" };
