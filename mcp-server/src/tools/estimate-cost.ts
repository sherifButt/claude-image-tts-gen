...
import type { Modality, ProviderId, Tier, VideoTier } from "../providers/types.js";
...
export interface EstimateCostOutput {
  ...
  cheapest: EstimateRow | null;
  cheapestBatch: EstimateRow | null;
  text: string;
  currency: string;
}

export function estimateCostDryRun(args: EstimateCostArgs): EstimateCostOutput {
  ...
  const slots = listAvailable(args.modality).filter((s) => {
    if (args.provider && s.provider !== args.provider) return false;
    if (args.tier && s.tier !== args.tier) return false;
    return true;
  });

  if (slots.length === 0) {
    throw new Error(
      `No implemented ${args.modality} providers match the filter. Try without --provider/--tier or run list_providers.`,
    );
  }
  ...
  for (const slot of slots) {
    ...
    const params: Record<string, unknown> = slot.params;

    const standard: CostEstimate = estimateCost(
      { provider: slot.provider, model: slot.model, modality: args.modality, params },
      units,
    );
    ...
    if (slot.batchable) {
      try {
        ...
        if (batch.isBatchPrice) batchTotal = batch.total;
      } catch {
        batchTotal = null;
      }
    }
...
function resolveUnits(args: EstimateCostArgs): number {
  if (args.modality === "image") return args.count ?? 1;
  if (args.modality === "video") return args.seconds ?? 5;
  if (args.chars !== undefined) return args.chars;
  if (args.text !== undefined) return args.text.length;
  return 0;
}
