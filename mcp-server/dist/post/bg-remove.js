...
async function loadBackgroundRemoval() {
    try {
        ...
        throw new StructuredError("CONFIG_ERROR", "@imgly/background-removal-node is not installed in the plugin folder", "The plugin's bootstrap should install this automatically on first start. " +
...
export function suggestBgRemoveOutputPath(inputPath) {
...
export async function removeBackground(inputPath, outputPath) {
    ...
    const onProgress = (key, current, total) => {
        ...
        if (phase !== lastPhase) {
            lastPhase = phase;
            process.stderr.write(`[claude-image-tts-gen] bg-remove: ${phase} (${current}/${total})\n`);
        }
    };
