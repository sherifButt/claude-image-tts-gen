...
import { listPresets, saveStylePreset, saveVoicePreset, } from "./tools/presets.js";
...
async function main() {
    try {
        ...
        if (values["list-presets"] !== undefined) {
            ...
            const result = await listPresets({ kind: kind });
            process.stdout.write(result.text + "\n");
            process.exit(0);
        }
        ...
        if (values.regenerate) {
            const result = await regenerate({ path: values.regenerate, outputPath: values.output }, config);
        ...
        if (values.avatar) {
            ...
            if (values.tier !== undefined && !isAvatarTier(values.tier)) {
                throw new Error(`Invalid --tier for --avatar: ${values.tier} (use draft | low | normal | high | ultra)`);
            }
        ...
        if (values.iterate) {
            ...
            const result = await iterate({ path: values.iterate, adjustment: values.adjustment, outputPath: values.output }, config);
            process.stdout.write(JSON.stringify(result) + "\n");
            process.exit(0);
        }
        if (values.variants) {
            ...
            if (Number.isNaN(n))
                throw new Error(`Invalid --n: ${values.n}`);
        ...
        if (values["post-process"]) {
            const presets = (values.presets ?? "").split(",").map((s) => s.trim()).filter(Boolean);
        ...
        if (values["set-budget-daily"] !== undefined ||
            values["set-budget-weekly"] !== undefined ||
            values["set-budget-monthly"] !== undefined) {
            const parseCap = (v) => {
                if (v === undefined)
                    return undefined;
                if (v === "null" || v === "none" || v === "off")
                    return null;
                const n = Number(v);
                if (Number.isNaN(n))
                    throw new Error(`Invalid budget value: ${v}`);
                return n;
            };
