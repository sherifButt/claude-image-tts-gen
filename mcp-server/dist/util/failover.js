import { resolveSlot } from "../providers/registry.js";
...
function hasKeyFor(providerId, config) {
    switch (providerId) {
        case "google":
...
export function getFailoverOrder(modality, preferred, config) {
...
export async function withFailover(opts) {
    const order = opts.pinToPreferred
        ? [opts.preferredProvider]
        : getFailoverOrder(opts.modality, opts.preferredProvider, opts.config);
    if (order.length === 0) {
        throw new StructuredError("CONFIG_ERROR", `No provider with a configured API key for ${opts.modality}`, `Set at least one of GEMINI_API_KEY, OPENAI_API_KEY, OPENROUTER_API_KEY, ELEVENLABS_API_KEY.`);
    }
    ...
    for (let i = 0; i < order.length; i++) {
        ...
        try {
            ...
            if (i === 0) {
                throw new StructuredError("VALIDATION_ERROR", `${providerId} does not offer ${opts.modality} at ${opts.tier} tier in this version`, `Try a different --tier, or omit --provider to let failover pick.`);
            }
            continue;
        }
        if (i === 0) {
