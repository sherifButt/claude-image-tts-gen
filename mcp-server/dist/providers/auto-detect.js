...
export async function probeVoicebox(baseUrl) {
    return probe(`${baseUrl.replace(/\/$/, "")}/health`);
}
export async function probeLocal(baseUrl) {
...
export async function probePocketTts(baseUrl) {
    return probe(`${baseUrl.replace(/\/$/, "")}/health`);
}
 ...
 */
export async function applyAutoDetection(config) {
    const tasks = [];
    if (config.voiceboxAutoProbe) {
        tasks.push(probeVoicebox(config.voiceboxBaseUrl).then((reachable) => {
            if (reachable) {
                config.voiceboxEnabled = true;
                log(config, "info", `voicebox: auto-detected at ${config.voiceboxBaseUrl} (set VOICEBOX_ENABLED=false to opt out)`);
            }
            else {
    ...
    if (config.localAutoProbe) {
        tasks.push(probeLocal(config.localBaseUrl).then((reachable) => {
            if (reachable) {
                config.localEnabled = true;
                log(config, "info", `local: auto-detected at ${config.localBaseUrl} (set LOCAL_ENABLED=false to opt out)`);
            }
            else {
    ...
    if (config.pocketTtsAutoProbe) {
        tasks.push(probePocketTts(config.pocketTtsBaseUrl).then((reachable) => {
            if (reachable) {
                config.pocketTtsEnabled = true;
                log(config, "info", `pocket-tts: auto-detected at ${config.pocketTtsBaseUrl} (set POCKET_TTS_ENABLED=false to opt out)`);
            }
            else {
