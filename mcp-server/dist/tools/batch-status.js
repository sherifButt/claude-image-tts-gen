...
export async function batchStatus(args, config) {
    if (args.list) {
    ...
    if (!args.jobId) {
        throw new StructuredError("VALIDATION_ERROR", "jobId is required (or pass list:true to list all jobs)", "Re-run with --job-id <id> from a previous batch_submit, or --list.");
    }
    ...
    const transitioned = previousStatus === "in_progress" &&
...
async function processCompletedJob(job, results, config) {
    ...
    for (let i = 0; i < job.prompts.length; i++) {
        ...
        const cost = estimateCost({ provider: job.provider, model: job.model, modality: job.modality, params: prompt.params }, units, { useBatch: true });
...
function renderJobsList(jobs) {
    ...
    for (const j of jobs) {
        lines.push(`  ${j.jobId}  ${j.status.padEnd(15)}  ${j.provider}/${j.tier}  ${j.prompts.length} prompts  ` +
