...
export async function pickVariant(args) {
    if (!args.keeper) {
    ...
    if (!args.variants || args.variants.length === 0) {
        throw new StructuredError("VALIDATION_ERROR", "variants array is required", "Pass the list of all variant file paths returned by the variants tool.");
    }
    if (!args.variants.includes(args.keeper)) {
        throw new StructuredError("VALIDATION_ERROR", `keeper "${args.keeper}" is not in the variants list`, "Pass the keeper as one of the variant paths.");
    }
    const baseDir = dirname(args.keeper);
    const trashDir = join(baseDir, TRASH_DIR_NAME);
    const trashed = [];
    for (const variant of args.variants) {
        if (variant === args.keeper)
            continue;
