...
const MAX_N = 9;
export async function variants(args, config) {
    if (!args.prompt || args.prompt.trim().length === 0) {
        throw new StructuredError("VALIDATION_ERROR", "prompt is required", "Pass a non-empty prompt.");
    }
    const n = args.n ?? 4;
    if (n < 2 || n > MAX_N) {
        throw new StructuredError("VALIDATION_ERROR", `n must be between 2 and ${MAX_N} (got ${n})`, `Try n=4 (default) or up to ${MAX_N} for a 3x3 sheet.`);
    }
    // Run n generations in parallel.
    const results = await Promise.all(Array.from({ length: n }, () => generateImage({
        prompt: args.prompt,
