...
import { imageDimensions, imageThumbDataUri, videoDimensions, videoPosterDataUri, } from "../post/thumbnail.js";
const EXT_KIND = {
...
async function collectFiles(dirs) {
    ...
    for (const dir of dirs) {
        ...
        for (const name of entries) {
            if (name.startsWith("."))
                continue; // sidecars are dotfiles
...
function readInput(meta) {
    if (meta.tool === "generate_image") {
        ...
        const bits = [
            ...
            i.background ? `bg ${i.background}` : null,
        ].filter(Boolean);
...
async function attachThumb(item, file) {
    if (item.kind === "image")
        item.thumb = await imageThumbDataUri(file);
    else if (item.kind === "video")
        item.thumb = await videoPosterDataUri(file);
    else
        item.thumb = null;
}
async function attachDims(item, file) {
    const d = item.kind === "image"
        ? await imageDimensions(file)
        : item.kind === "video"
            ? await videoDimensions(file)
            : null;
    if (d)
        item.dims = `${d.width}×${d.height}`;
}
export async function gallery(args, config) {
    const dirs = (args.dirs && args.dirs.length > 0
        ? args.dirs
        : [config.imageOutputDir, config.audioOutputDir, config.videoOutputDir]).map((d) => (isAbsolute(d) ? d : resolve(process.cwd(), d)));
    ...
    if (modFilter)
        items = items.filter((i) => i.modality === modFilter);
    if (args.provider)
        items = items.filter((i) => i.provider === args.provider);
    if (args.model)
        items = items.filter((i) => (i.model ?? "").includes(args.model));
    ...
    for (const item of items) {
        ...
        if (wantThumbs) {
            ...
            if (item.kind === "image" && item.thumb === null)
                sharpMissing = true;
        }
    }
    const counts = {
    ...
    if (sharpMissing) {
        lines.push(`Note: sharp not installed — images fall back to full-res <img> (heavier). ` +
...
// ---------------------------------------------------------------------------
function renderGalleryHtml(items, meta) {
