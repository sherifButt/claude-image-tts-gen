...
export async function generateSpeech(args, config, opts = {}) {
    ...
    if (args.referenceAudioPath) {
        if (requestedProvider !== "local" && requestedProvider !== "pocket-tts") {
            throw new StructuredError("VALIDATION_ERROR", `referenceAudio (voice cloning) is only supported on provider=pocket-tts or provider=local. Got provider=${requestedProvider}.`, requestedProvider === "elevenlabs"
        ...
        if (requestedProvider === "pocket-tts") {
            const { PocketTtsProvider, cloningUnavailableError } = await import("../providers/pocket-tts.js");
