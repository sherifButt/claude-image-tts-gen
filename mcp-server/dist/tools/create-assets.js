...
export function checkBatchAvailability(args) {
    ...
    const sync = estimateCost({ provider: providerId, model: slot.model, modality: args.modality, params: slot.params }, units);
    const batch = estimateCost({ provider: providerId, model: slot.model, modality: args.modality, params: slot.params }, units, { useBatch: true });
    return {
        available: true,
...
export async function createAssets(args, config) {
    ...
    if (requested === "batch") {
        if (!availability.available) {
            throw new StructuredError("VALIDATION_ERROR", `mode=batch requested but batch is not available: ${availability.reason}`, `Use mode=sync, or pick a batch-capable provider.`);
        }
        mode = "batch";
    }
    ...
    const results = await Promise.all(args.prompts.map((p) => args.modality === "image"
        ? generateImage({
            prompt: p.text,
            provider: args.provider,
            tier: args.tier,
    ...
    if (availability.available && availability.savings && availability.savings > 0) {
        lines.push(`Tip: batch mode would have cost ${availability.currency} ${availability.batchCost?.toFixed(4)} ` +
