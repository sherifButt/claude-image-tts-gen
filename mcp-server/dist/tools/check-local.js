import { listLocalModels } from "../providers/local.js";
import { StructuredError } from "../util/errors.js";
export async function checkLocal(config) {
    let models;
    try {
        ...
        throw new StructuredError("PROVIDER_ERROR", `Could not reach local server at ${config.localBaseUrl}: ${message}`, `Start an OpenAI-compatible server (Kokoro-FastAPI, Speaches, Orpheus-FastAPI, LM Studio, ...) or set LOCAL_BASE_URL to its base URL. Example:
  ...
  export LOCAL_BASE_URL=http://localhost:8880/v1`);
    ...
    const lines = models.map((m) => `  ${m.id}  [${m.likelyCapability}]`);
...
function guessBackend(baseUrl, models) {
    ...
    if (/kokoro/.test(ids) || /:8880/.test(baseUrl))
        return "kokoro-fastapi";
    if (/orpheus/.test(ids) && /:5005/.test(baseUrl))
        return "orpheus-fastapi";
    if (/chatterbox/.test(ids))
        return "chatterbox";
    if (/speaches|piper/.test(ids) || /:8000/.test(baseUrl))
        return "speaches";
    if (/:1234/.test(baseUrl))
        return "lm-studio";
    return "unknown";
}
