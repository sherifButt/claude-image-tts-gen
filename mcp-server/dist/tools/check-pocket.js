import { POCKET_TTS_DEFAULT_VOICE, POCKET_TTS_VOICES, PocketTtsProvider, } from "../providers/pocket-tts.js";
/**
 ...
 */
export async function checkPocket(config) {
    ...
    if (!reachable) {
        fixes.push(`Start the server: \`pocket-tts serve\` (listens on :8000 by default). Install it with \`pip install pocket-tts\`.`);
    ...
    if (!builtInVoices) {
        ...
        fixes.push("Voice cloning is unavailable — the weights are a gated HuggingFace repo. Accept the terms at https://huggingface.co/kyutai/pocket-tts, run `hf auth login`, then restart the server.");
        fixes.push("Until then, built-in voices still work. generate_speech will REFUSE a cloning request rather than quietly substituting a stock voice.");
    }
    if (!config.pocketTtsEnabled) {
        fixes.push("Reachable but not enabled for automatic selection. Set POCKET_TTS_ENABLED=true, or just pass --provider pocket-tts explicitly (always allowed).");
    }
    return render({
        ok: builtInVoices,
        baseUrl,
